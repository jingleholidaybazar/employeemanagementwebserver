...
const AdminAttendance = () => {
    ...
    const fetchAttendance = async () => {
      ...
      try {
        const response = await axios.get(
          "https://employeemanagment-production-c550.up.railway.app/api/attendance/allAttendance",
  ...
  const handleMarkLeave = async () => {
    ...
    if (currentTime > cutoffTime) {
      ...
      try {
        const response = await axios.get(
          "https://employeemanagment-production-c550.up.railway.app/api/automark/auto-mark",
          { headers: { Authorization: `Bearer ${token}` } }
        );
